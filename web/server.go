...
func (s *Server) ListenAndServe(addr string) error {
	...
	ignoredUrls := []string{"/bundles/", "/fonts/", "/debug/vars", "/favicon", "/robots", "/images"}
...
func (s *Server) loadWebpackAssets(webpackAssetsFile string, subPath string) {
	...
	if data, err := ioutil.ReadFile(webpackAssetsFile); err == nil {
		...
		// 			key := fmt.Sprintf("%s.%s", entry, ty)
		// 			if entry == "" {
		// 					key = ty;
		// 			}
		// 			log.Infof("[Server] assets %s => %s", key, target);
		// 			newMappings[key] = target
		// 		}
		// 	}
		// 	log.Infof("[Server] assets Map", newMappings);
		// 	s.EnableExtraAssetsMapping(newMappings)
		// 	log.Infof("[Server] LLLLLoaded webpack assets from %s, duration=%s", webpackAssetsFile, time.Now().Sub(start))
		// } else {
		// 	log.Errorf("[Server] Failed to decode the web pack assets, %s", err)
		// }

		var packMappings map[string]string

		if err := json.Unmarshal(data, &packMappings); err == nil {
			for entry, file := range packMappings {
				log.Infof("[Server] asset %s -> %s", entry, file);
			}
			s.EnableExtraAssetsMapping(packMappings);
			log.Infof("[Server] LLLLLoaded webpack assets from %s, duration=%s", webpackAssetsFile, time.Now().Sub(start))
